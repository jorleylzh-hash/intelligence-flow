...
if st.session_state.page == 'home':
    ...
    if st.session_state.auth_status:
        ...
        with col2:
            st.markdown("""<div style="background:#1e293b; padding:30px; border-radius:10px; border:1px solid #334155; text-align:center;"><h3 style="color:white;">Acesso Institucional</h3></div>""", unsafe_allow_html=True)
            if st.session_state.login_step == 1:
                ...
                if st.button("ENVIAR CÓDIGO", type="primary", use_container_width=True):
                    if "@" in email_input:
                        ...
                        except: st.warning(f"Debug: Código {code}")
                        st.session_state.login_step = 2
                        st.rerun()
            elif st.session_state.login_step == 2:
                st.info(f"Código enviado para: {st.session_state.otp_email}")
                user_code = st.text_input("Código", max_chars=4)
